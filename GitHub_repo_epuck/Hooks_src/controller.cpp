...
CConnectivityBuzzControllerEPuck::SWheelTurningParams::SWheelTurningParams() :
...
void CConnectivityBuzzControllerEPuck::SWheelTurningParams::Init(TConfigurationNode& t_node) {
   try {
      TurningMechanism = NO_TURN;
      CDegrees cAngle;
...
static int BuzzGoToC(buzzvm_t vm) {
   ...
   buzzvm_gload(vm);
   /* Call function */
   reinterpret_cast<CConnectivityBuzzControllerEPuck*>(buzzvm_stack_at(vm, 1)->u.value)->SetWheelSpeedsFromVector(cDir);
...
int BuzzSetWheels(buzzvm_t vm) {
   ...
   buzzvm_gload(vm);
   /* Call function */
   reinterpret_cast<CConnectivityBuzzControllerEPuck*>(
...
int C_OneShotPathPlanner(buzzvm_t vm) {
  ...
  /* Get pointer to controller user data */
   ...
   CConnectivityBuzzControllerEPuck& cContr = *reinterpret_cast<CConnectivityBuzzControllerEPuck*>(buzzvm_stack_at(vm, 1)->u.value);
...
CConnectivityBuzzControllerEPuck::CConnectivityBuzzControllerEPuck() :
   m_pcWheels(NULL),
...
void CConnectivityBuzzControllerEPuck::Init(TConfigurationNode& t_node) {
   try {
      ...
      THROW_ARGOSEXCEPTION_NESTED("Error initializing the Buzz controller for the epuck", ex);
   }
}
...
void CConnectivityBuzzControllerEPuck::UpdateSensors() {
   /*
...
void CConnectivityBuzzControllerEPuck::SetWheelSpeedsFromVector(const CVector2& c_heading) {
...
void CConnectivityBuzzControllerEPuck::SetWheels(Real f_left_speed,
                                         Real f_right_speed) {
...
std::vector<std::vector<double>> CConnectivityBuzzControllerEPuck::OneShotPathPlanner(float* start_end_time){
  // for(int i =0; i< 5; i++){
...
std::vector<std::vector<double>> CConnectivityBuzzControllerEPuck::import_map(std::string m_map_file_name)
{
  std::string line;
...
std::vector<std::vector<double>> CConnectivityBuzzControllerEPuck::import_3dmap(std::string m_map_file_name)
{
  std::string line;
  std::ifstream m_map_file (m_map_file_name);
  int map_height = 0;
...
buzzvm_state CConnectivityBuzzControllerEPuck::RegisterFunctions() {
...
std::string CConnectivityBuzzControllerEPuck::GetMapFName(){
  return strmapFName;
}

/****************************************/
/****************************************/
svg::Point CConnectivityBuzzControllerEPuck::visualize_point(float* state, svg::Dimensions dims)
{
...
svg::Point CConnectivityBuzzControllerEPuck::visualize_point(double* state, svg::Dimensions dims)
{
  double x = ((state[0]-(half_map_height/2))-MIN_X)/(MAX_X-MIN_X) * dims.width; 
  double y = ((state[1]-(half_map_length/2))-MIN_Y)/(MAX_Y-MIN_Y) * dims.height; 
  return svg::Point(x,y);
...
svg::Point CConnectivityBuzzControllerEPuck::visualize_point(std::vector<double> state, svg::Dimensions dims)
{
  double x = ((state[0]-(half_map_height/2)) -MIN_X)/(MAX_X-MIN_X) * dims.width; 
  double y = ((state[1]-(half_map_length/2))-MIN_Y)/(MAX_Y-MIN_Y) * dims.height; 
  return svg::Point(x,y);
...
svg::Point CConnectivityBuzzControllerEPuck::visualize_point_woScale(std::vector<double> state, svg::Dimensions dims)
{
  double x = ((state[0]+(half_map_height/2)) -MIN_X)/(MAX_X-MIN_X) * dims.width; 
  double y = ((state[1]+(half_map_length/2))-MIN_Y)/(MAX_Y-MIN_Y) * dims.height; 
  return svg::Point(x,y);
...
svg::Point CConnectivityBuzzControllerEPuck::visualize_point(ob::State* state, svg::Dimensions dims)
{
...
REGISTER_CONTROLLER(CConnectivityBuzzControllerEPuck, "buzz_connectivity_controller_epuck");

#ifdef ARGOS_DYNAMIC_LIBRARY_LOADING
#include <argos3/plugins/robots/e-puck/simulator/epuck_entity.h>
REGISTER_BUZZ_ROBOT(CEPuckEntity);
#endif
