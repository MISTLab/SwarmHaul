...
int C_OneShotPathPlanner(buzzvm_t vm) {
  if(buzzdarray_size(vm->lsyms->syms)< 9 || buzzdarray_size(vm->lsyms->syms) > 9){
    fprintf(stderr, "[ROBOT %u] expected 8 args for planner but received : %u \n\n",vm->robot, 
            buzzdarray_size(vm->lsyms->syms)-1);
    return buzzvm_ret0(vm);
  }
  float start_end_time[8];
  ...
  /* Get pointer to controller user data */
   ...
   for(uint32_t i=0; i< controls.size();++i){
    ...
    //  Create table for i-th face
    buzzvm_pusht(vm);
    buzzobj_t ith_pose = buzzvm_stack_at(vm, 1);
    buzzvm_pop(vm);

    //  Fill in the point
    buzzvm_push(vm, ith_pose);
    ...
    //  Store ith pose in the path table
