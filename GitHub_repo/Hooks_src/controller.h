...
protected:
   ...
   int Deep_planning;
    ...
    //       std::vector<std::pair <int,std::vector<float>>> *restictz, float &res, int grid_to) :
    //       ob::StateValidityChecker(si),gridToCheck(grid_to),map_resolution(res) {
    //       Grid_obst = Grid_map;
    ...
    //         if(state2D->getX()*(1/map_resolution) < ref_g_obst.size()-1 &&
    //             state2D->getX()*(1/map_resolution) >= 0 &&
    //             state2D->getY()*(1/map_resolution) < ref_g_obst[state2D->getX()].size()-1 &&
    //             state2D->getY()*(1/map_resolution) >= 0){
    //           for(int i=-gridToCheck; i < gridToCheck; ++i){
    //             for(int j=-gridToCheck; j < gridToCheck;++j){
    //               if((state2D->getX())*(1/map_resolution)+i < ref_g_obst.size() &&
    //                    (state2D->getX())*(1/map_resolution)+i >= 0 &&
    //                    (state2D->getY())*(1/map_resolution)+j < ref_g_obst[state2D->getX()+i].size() &&
    //                    (state2D->getY())*(1/map_resolution)+j >=0 ){
    //                 if(ref_g_obst[(state2D->getX())*(1/map_resolution)+i][(state2D->getY())*(1/map_resolution)+j]) return false;

    //                 for(size_t k=0;k<m_restrictz->size(); ++k){
    //                   std::vector<float> c_zone = (*m_restrictz)[k].second;

    //                   if( (state2D->getX()+i) >= c_zone[0] && (state2D->getX()+i) <= c_zone[1]){
    //                     if( (!c_zone[4]) && (state2D->getY()+j) < c_zone[3]){
    //                       // printf(" Invalid \n");
    //                       return false;
    //                     }
    //                     else if(c_zone[4] && (state2D->getY()+j) > c_zone[2]){
